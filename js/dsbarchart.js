d3.csv('https://raw.githubusercontent.com/qinlinc/CSC3007-project/main/prod_cons.csv').then(data => {
    ...
    var svg = d3.select("#dsbarchart")
        .append('svg')
        .attr('class', 'svg')
        .attr('width', label_width + bar_width + bar_width)
        .attr('height', height);

    const prodVal = data.map(d => parseInt(d.total_production))
    const prodMax = Math.max(...prodVal)
    const consVal = data.map(d => parseInt(d.total_consumption))
    const consMax = Math.max(...consVal)
    const max = Math.max(prodMax,consMax)
    xLHS.domain([0, max])
    xRHS.domain([0, max])
